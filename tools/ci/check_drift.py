...
A request failure for one package does not abort the rest of the audit.

Does not import, rewrite, or bump any proto package. Fixing drift is a
byte-copy of that source's proto/ from default-branch HEAD (BUILD files
here stay contracts-local), then updating that row's Source SHA.
"""
...
from ledger import github_repo, parse_ledger  # noqa: E402

API = "https://api.github.com"
USER_AGENT = "fastverk-contracts-drift-check"
TIMEOUT = 30
...
def write_step_summary(results: list[dict]) -> None:
    ...
    lines = [
        ...
        "",
        "| Package | Status | Ahead | Ledger SHA | Source HEAD | Notes |",
        "| --- | --- | ---: | --- | --- | --- |",
    ]
    for r in results:
        if r["state"] == "in sync":
            ...
            status = "unreachable"
        ahead = r["ahead"] if r["state"] != "unreachable" else "—"
        lines.append(
            f"| `{r['module']}` | {status} | {ahead} "
...
def main() -> int:
    ...
    if drifted:
        print(
            f"drift check FAILED: {len(drifted)} package(s) ahead of ledger SHA"
        )
        print(
            "fix: byte-copy that source's proto/ from default-branch HEAD "
            "(keep contracts-local BUILD files), then bump the row's Source SHA"
        )
        return 1
    print("drift check OK: no drifted packages")
    return 0
